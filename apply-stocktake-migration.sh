...
STOCKTAKE_EXISTS=$(docker compose exec -T db psql -U $POSTGRES_USER -d $POSTGRES_DB -c "\dt stocktakes" 2>/dev/null | grep -c "stocktakes" || echo "0")
...
docker compose exec -T db psql -U $POSTGRES_USER -d $POSTGRES_DB -c "\dt stocktake*"
