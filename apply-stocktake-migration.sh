...
STOCKTAKE_EXISTS=$(docker compose exec -T db psql -U ${POSTGRES_USER:-abparts_user} -d ${POSTGRES_DB:-abparts_dev} -c "\dt stocktakes" 2>/dev/null | grep -c "stocktakes" || echo "0")
...
docker compose exec -T db pg_dump -U ${POSTGRES_USER:-abparts_user} -d ${POSTGRES_DB:-abparts_dev} > $BACKUP_FILE
...
docker compose exec -T db psql -U ${POSTGRES_USER:-abparts_user} -d ${POSTGRES_DB:-abparts_dev} -c "\dt stocktake*"
