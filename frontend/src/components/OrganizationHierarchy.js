...
const OrganizationHierarchy = ({ onOrganizationSelect, selectedOrganizationId }) => {
  ...
  const loadHierarchy = async () => {
    ...
    try {
      ...
      setError(t('organizationHierarchy.failedToLoad'));
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="flex items-center space-x-2">
              ...
              {!org.is_active && (
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                  {t('organizationHierarchy.inactive')}
                </span>
              )}
            </div>
            <div className="flex items-center space-x-2 mt-1">
              ...
              {hasChildren && (
                <span className="text-xs text-gray-500">
                  {visibleChildren.length} {visibleChildren.length === 1 ? t('organizationHierarchy.child') : t('organizationHierarchy.children')}
                </span>
              )}
            </div>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-medium text-gray-900">{t('organizationHierarchy.title')}</h3>
          <p className="text-sm text-gray-500">
            {t('organizationHierarchy.subtitle')}
          </p>
        </div>
        <button
        ...
        >
          ...
          {t('organizationHierarchy.refresh')}
        </button>
      </div>
      ...
      <div className="flex items-center space-x-4">
        <label className="flex items-center">
          ...
          />
          <span className="ml-2 text-sm text-gray-700">{t('organizationHierarchy.includeInactive')}</span>
        </label>
        <div className="flex items-center space-x-2">
          <button
          ...
          >
            {t('organizationHierarchy.expandAll')}
          </button>
          ...
          <button
            ...
            className="text-sm text-indigo-600 hover:text-indigo-500"
          >
            {t('organizationHierarchy.collapseAll')}
          </button>
        </div>
      </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          {hierarchyData.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              {t('organizationHierarchy.noOrganizations')}
            </div>
          ) : (
      ...
      <div className="bg-gray-50 rounded-lg p-4">
        <h4 className="text-sm font-medium text-gray-900 mb-3">{t('organizationHierarchy.organizationTypes')}</h4>
        <div className="grid grid-cols-2 gap-3">
          {Object.entries(getOrganizationTypeConfig(t)).map(([type, config]) => (
