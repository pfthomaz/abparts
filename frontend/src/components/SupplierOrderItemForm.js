// frontend/src/components/SupplierOrderItemForm.js

import React, { useState, useEffect } from 'react';
import { useAuth } from '../AuthContext';

function SupplierOrderItemForm({ supplierOrders = [], parts = [], initialData = {}, onSubmit, onClose }) {
  ...
  // Only Oraseas roles can create/edit supplier order items
  const canModify = user && (user.role === "Oraseas Admin" || user.role === "Oraseas Inventory Manager");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label htmlFor="supplier_order_id" className="block text-sm font-medium text-gray-700 mb-1">
          Supplier Order
        </label>
        <select
        ...
        >
          <option value="">Select a Supplier Order</option>
          {supplierOrders.map((order) => (
            <option key={order.id} value={order.id}>Order {order.id.substring(0, 8)} - {order.supplier_name} ({new Date(order.order_date).toLocaleDateString()})</option>
          ))}
        </select>
      </div>
...
export default SupplierOrderItemForm;
