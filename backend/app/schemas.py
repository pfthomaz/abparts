...
class MaintenanceExecutionBase(BaseModel):
    ...
    machine_hours_at_service: Decimal = Field(..., decimal_places=2, ge=0)
    next_service_due_hours: Optional[Decimal] = Field(None, decimal_places=2, ge=0)
    status: MaintenanceExecutionStatusEnum = MaintenanceExecutionStatusEnum.COMPLETED
    notes: Optional[str] = None
...
class MaintenanceChecklistCompletionCreate(MaintenanceChecklistCompletionBase):
    pass
...
# Old StockAdjustmentResponse removed - see new version below with StockAdjustmentListResponse
...
# --- Stock Adjustment Schemas (Redesigned) ---
